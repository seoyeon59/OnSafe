...
def benchmark_avg(score_history: list):
    ...
    global event_counts, event_totals, event_mins, event_maxs
    ...
    stage = "6. 최근 3개 score 평균  O(1)"

    for _ in range(REPEAT):
        t0      = time.perf_counter()
        last3   = score_history[-3:]       # O(1) 리스트 슬라이싱
        avg_val = sum(last3) / len(last3)  # O(1) 고정 크기 3
        ...
        print(
            ...
            f"{stage:<50}"
            f"  elapsed={elapsed*1000:8.3f}ms"
            f"  avg={avg_time*1000:8.3f}ms"
            f"  (n={n:4d})"
            f"  avg_score={avg_val:.2f}"
        )
...
def run_pipeline(video_path):
    ...
    print("=" * 120)
    print(f"[EVENT] 파이프라인 완료  총 프레임={frame_idx}  추론={inference_count}  전체={total_elapsed:.2f}s")

    # 최근 3개 평균 벤치마크 — 요약 출력 전에 실행해야 6번이 요약에 포함됨
  ...
  구간 6  최근 3개 score 평균        O(1)         고정 크기 슬라이싱
  ─────────────────────────────────────────────────────────
