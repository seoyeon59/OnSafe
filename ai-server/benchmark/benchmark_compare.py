...
COLLECTION_RESULT = "onsafe_ai_results"
COLLECTION_ALL    = "onsafe_all_data"
# ============================================================
...
# ============================================================
# Redis 전용 파이프라인 — 피처 + AI 결과 저장 후 최신 3개 평균
# ============================================================
def run_pipeline_redis_only(video_path, r):
    print(f"\n▶️  [Redis 전용] [{video_path}] 파이프라인 시작\n")

    model  = joblib.load(MODEL_PATH)
    scaler = joblib.load(SCALER_PATH)

    redis_key_features = "onsafe:features"
    redis_key_results  = "onsafe:results"
    r.delete(redis_key_features)
    r.delete(redis_key_results)
    ...
    time_stats = {
        ...
        "6. Redis — 피처 저장  O(1)":    0.0,
        "7. Redis — AI결과 저장  O(1)":  0.0,
    }
    ...
    while cap.isOpened():
        ...
        if len(frame_buffer) == WINDOW_SIZE and (frame_idx % STRIDE == 0):
            ...
            feature_dict = {col: float(df_win[col].iloc[-1]) for col in FEATURE_COLUMNS}
            ...
            r.lpush(redis_key_features, json.dumps(redis_payload))
            time_stats["6. Redis — 피처 저장  O(1)"] += time.perf_counter() - t5

            # Redis — AI 결과 저장 O(1)
            t6 = time.perf_counter()
            r.lpush(redis_key_results, json.dumps({
                "frame": frame_idx,
                "score": score,
                "level": level,
                "timestamp": ts_now,
            }))
            time_stats["7. Redis — AI결과 저장  O(1)"] += time.perf_counter() - t6
    ...
    print("\n" + "=" * 60)
    print("  📊 [Redis 전용] 파이프라인 구간별 시간 프로파일링 결과")
    print("=" * 60)
    ...
    # 최신 3개 AI 결과 평균 측정
    benchmark_redis_avg(r, redis_key_results, inference_count)

    return redis_key_features, redis_key_results, inference_count


def benchmark_redis_avg(r, redis_key_results, inference_count):
    if inference_count < 3:
        print("\n⚠️  추론 횟수 3회 미만 — Redis 평균 벤치마크 생략")
        return

    REPEAT = 100
    times  = []

    print(f"\n⏳ Redis 최신 3개 평균 벤치마크 ({REPEAT}회 반복)...\n")

    for _ in range(REPEAT):
        t0   = time.perf_counter()
        raw  = r.lrange(redis_key_results, 0, 2)          # O(1)
        scores = [json.loads(x)["score"] for x in raw]    # O(1) 고정 크기 3
        avg  = sum(scores) / len(scores)                   # O(1)
        times.append(time.perf_counter() - t0)

    avg_t = sum(times) / REPEAT
    min_t = min(times)
    max_t = max(times)

    print("=" * 60)
    print("  📊 [Redis 전용] 최신 3개 score 평균 조회 결과")
    print("=" * 60)
    print(f"  조회 방식 : LRANGE 0 2  → O(1) 인메모리")
    print(f"  평균 score: {avg:.2f}")
    print(f"  평균 시간 : {avg_t*1000:.3f} ms")
    print(f"  최소 시간 : {min_t*1000:.3f} ms")
    print(f"  최대 시간 : {max_t*1000:.3f} ms")
    print("=" * 60)


# ============================================================
# 기존 Redis + Firestore 비교 파이프라인 (유지)
# ============================================================
def run_pipeline(video_path):
    ...
    while cap.isOpened():
        ...
        if len(frame_buffer) == WINDOW_SIZE and (frame_idx % STRIDE == 0):
            ...
            t6 = time.perf_counter()
            db.collection(COLLECTION_RESULT).document(f"frame_{frame_idx}").set({
                "frame": frame_idx, "score": score,
                "level": level,    "timestamp": ts_now,
            })
            time_stats["7. Firestore — AI결과만 저장"] += time.perf_counter() - t6

            t7 = time.perf_counter()
            all_data = {"frame": frame_idx, "score": score,
                        "level": level,    "timestamp": ts_now}
...
def benchmark_read_and_avg(r, redis_key, db, inference_count):
  ...
  [케이스 B] Firestore(피처 + AI결과 전부)
    ...
    print(f"     최대 : {b_max*1000:.3f} ms")# 케이스 C: Redis만 (이미 측정된 값 가져오기)

    redis_only_key = "onsafe:results"
    case_c_times = []
    for _ in range(REPEAT):
        t0     = time.perf_counter()
        raw    = r.lrange(redis_only_key, 0, 2)
        scores = [json.loads(x)["score"] for x in raw]
        avg_c  = sum(scores) / len(scores)
        case_c_times.append(time.perf_counter() - t0)

    c_avg, c_min, c_max = stats(case_c_times)

    print("""
  [케이스 C] Redis만 (피처 + AI결과 전부)
    └─ 결과 : Redis LRANGE → O(1), 인메모리, 네트워크 없음
    """)
    print(f"     평균 : {c_avg*1000:.3f} ms")
    print(f"     최소 : {c_min*1000:.3f} ms")
    print(f"     최대 : {c_max*1000:.3f} ms")

    times_all = {"케이스 A (Redis+FS)": a_avg, "케이스 B (FS만)": b_avg, "케이스 C (Redis만)": c_avg}
    fastest   = min(times_all, key=times_all.get)
    slowest   = max(times_all, key=times_all.get)
    ratio     = times_all[slowest] / times_all[fastest]

    print(f"\n  ✅ {fastest}가 가장 빠름  (가장 느린 {slowest} 대비 약 {ratio:.1f}배)")
    print(f"\n  {'케이스':<25}  {'평균':>10}  {'최소':>10}  {'최대':>10}")
    print(f"  {'-'*55}")
    print(f"  {'A. Redis(피처)+FS(AI결과)':<25}  {a_avg*1000:>9.3f}ms  {a_min*1000:>9.3f}ms  {a_max*1000:>9.3f}ms")
    print(f"  {'B. FS(피처+AI결과 전부)':<25}  {b_avg*1000:>9.3f}ms  {b_min*1000:>9.3f}ms  {b_max*1000:>9.3f}ms")
    print(f"  {'C. Redis만':<25}  {c_avg*1000:>9.3f}ms  {c_min*1000:>9.3f}ms  {c_max*1000:>9.3f}ms")
...
if __name__ == "__main__":
    ...
    # 1. Redis 전용 파이프라인
    print("\n" + "=" * 60)
    print("  🔴 Redis 전용 파이프라인 실행")
    print("=" * 60)
    run_pipeline_redis_only(VIDEO, r)

    # 2. Redis + Firestore 비교 파이프라인
    print("\n" + "=" * 60)
    print("  🔵 Redis + Firestore 비교 파이프라인 실행")
    print("=" * 60)
